...
COMPOSE_TEST_RUN_APP = $(COMPOSE) run --rm app
...
lint-black: ## lint back-end python sources with black
	...
	@$(COMPOSE_TEST_RUN_APP) black src/gen4ralph tests
.PHONY: lint-black
...
lint-pylint: ## lint back-end python sources with pylint
	...
	@$(COMPOSE_TEST_RUN_APP) pylint src/gen4ralph tests
.PHONY: lint-pylint

lint-bandit: ## lint back-end python sources with bandit
	@echo 'lint:bandit started…'
	@$(COMPOSE_TEST_RUN_APP) bandit -qr src/gen4ralph
.PHONY: lint-bandit
